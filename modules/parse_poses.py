...
def get_root_relative_poses(inference_results):
    ...
    for pose_id in range(found_poses.shape[0]):
        if found_poses[pose_id, 3] == -1:  # skip pose if does not found neck
            continue
        only_keypoint = np.ones(num_kpt * 2, dtype=np.float32) * -1
        ...
        for kpt_id in range(num_kpt):
            if found_poses[pose_id, kpt_id * 3] != -1:
                ...
                #===
                if(map_id_to_panoptic[kpt_id]<2):
                    only_keypoint[map_id_to_panoptic[kpt_id] * 2]=x_2d
                    only_keypoint[map_id_to_panoptic[kpt_id] * 2+1]=y_2d
                if(map_id_to_panoptic[kpt_id]>2):
                    only_keypoint[(map_id_to_panoptic[kpt_id]-1) * 2]=x_2d
                    only_keypoint[(map_id_to_panoptic[kpt_id]-1) * 2+1]=y_2d
                #===
                ...
                #3**************************
                #print("label,x,y,conf,kpt_id",map_id_to_panoptic[kpt_id],x_2d,y_2d,conf,kpt_id)
        pose_2d[-1] = found_poses[pose_id, -1]
        #print("pose_2d[-1]",pose_2d[-1])
        poses_2d.append(pose_2d)
        only_keypoints.append(only_keypoint)
    ...
    return poses_3d, np.array(poses_2d), features.shape,np.array(only_keypoints)
...
def parse_poses(inference_results, input_scale, stride, fx, is_video=False):
    ...
    poses_3d, poses_2d, features_shape,_ = get_root_relative_poses(inference_results)
